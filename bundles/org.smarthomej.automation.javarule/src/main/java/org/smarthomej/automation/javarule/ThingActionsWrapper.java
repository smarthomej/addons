...
public class ThingActionsWrapper {
    ...
    /**
     * Get a {@link Proxy} object for the requsted thing action
     *
     ...
     * @return {@link Object} that proxies the corresponding interface (or null if not found}
     */
    ...
    private static class DynamicInvocationHandler implements InvocationHandler {
        ...
        public @Nullable Object invoke(Object proxy, Method method, @Nullable Object @Nullable [] args)
            ...
            logger.info("Invoked method: '{}' with args '{}'", methodName, args);
            Method[] methods = target.getClass().getDeclaredMethods();
            Optional<Method> targetMethod = Arrays.stream(methods) //
                    // .filter(m -> !Modifier.isStatic(method.getModifiers())) // only instance methods
                    // .filter(m -> methodName.equals(m.getName())) // method name
                    // .filter(m -> ClassUtils.isAssignable(ClassUtils.toClass(args), m.getParameterTypes())) // params
                    .filter(m -> m.toString().equals(methodSignature)).findFirst();
